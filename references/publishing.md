...
- **改文档 / 改代码**（加删规则、加删功能、修 SKILL.md、改脚本）→ **立即落盘**
  ——**不发版、不 push、不动 `version`、不动 CHANGELOG "已发布"段**
- **发版**（合并草稿 + bump 版本号 + commit + push）→ **只有一个触发条件**：
  用户**明确说"发版"**（含原触发词"更新上传 / 打包上传 / 上传到网盘"）
...
1. **`git` 不在默认 PATH 里**，且 WorkBuddy 沙箱 bash 环境异常（`ls`/`dirname`/`head`
   ...
   ```
   %USERPROFILE%\.workbuddy\binaries\PortableGit\versions\1.2.0\cmd\git.exe
   ```
...
### 触发 A（现在唯一的发版路径）：用户明确说"发版" / "更新上传"等触发词

- agent 立即执行：
  1. 把 CHANGELOG "待发布"段合并成新的 `### vX.Y.Z (今天)` 段，插到"已发布"段顶部
     ——**版本号和变更内容只在此时更新**，平时一概不动
  2. 清空"待发布"段（保留标记）
...
> 🚫 **2026-09-14 用户立规矩（最高优先级，压过本节其余条款）**
>
> **不许每次改动就发版。** 在用户说出"发版"之前，agent **不得** commit、不得 push、
> 不得改 `version`、不得往 CHANGELOG "已发布"段写东西 —— 改完代码安安静静落盘就行。
>
> 背景：2026-09-14 用户只要求改路径，agent 自作主张写了 v1.3.1 版本号 + CHANGELOG 条目
> 并试图 push，被当场叫停，那次 commit 已 `reset --soft` 撤销。
> **正确姿势**：改完 → 告诉用户"已落盘，等你说发版" → 收工。

### 触发 B：~~每天 23:59 automation 定时跑~~ —— **已取消（2026-09-14）**

- 用户明确取消：**不建 23:59 定时任务，agent 也不主动定时自查"要不要发版"**
- 取消原因：自动检查等于变相定期发版，且打扰用户
- 现状核对：本机 WorkBuddy `automation` 列表为空，确实没有注册过该任务
  （公司电脑那边若自建过，需手动删除）
- 现在**只有触发 A**（用户明确说"发版"）一条路径
